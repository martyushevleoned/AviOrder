...
4. Настроить HTTPS (для развёртывания)
   ...
   * Указать пароль от keystore.p12 и указать порт 443 в [prod.env](docker/production.env)
   * При использовании HTTP указать порт 80 и убрать строку ``spring_profiles_active=prod`` в [prod.env](docker/production.env)
